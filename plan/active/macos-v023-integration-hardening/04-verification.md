...
Current main SHA: `f7eaae1`

## Automated Checks

| Check | Windows | macOS | Evidence |
|---|---|---|---|
| `npm run check` | PASS on 2026-06-08 at `7ba8c40` | PASS on 2026-06-08 at `f7eaae1` | Windows local audit; Codex macOS rerun |
| `npm test` | PASS, 46 tests at `7ba8c40` | PASS, 46 tests at `f7eaae1` | Windows local audit; Codex macOS rerun |
| `cargo test` | FAIL, 47/48 at `7ba8c40` | PASS, 53 tests at `f7eaae1` | Windows hotkey assertion blocker on old SHA; Codex macOS rerun on current SHA |
...
- Re-run or confirm Windows local checks on `f7eaae1`.
- Re-run the Windows Trial Gate on `f7eaae1`.
- Confirm the macOS Adaptation Gate on `f7eaae1`.
